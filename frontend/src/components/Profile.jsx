...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:8000/main/profile/${username}/`, {
          headers: {
...
const handleLogout = async () => {
  ...
  try {
    await axios.post('http://localhost:8000/auth/api/logout/', { refresh_token: refreshToken }, {
      headers: {
  ...
  return (
    <div>
      ...
      <div>
        ...
        <p><strong>Number of Unique Accepted Submissions:</strong> {userData.num_unique_solved_problems}</p>
      </div>
